...
{
    [TestFixture]
    public class RavenStandardAnalyzerTests : RavenTestBase
    {
        [Test]
        ...
        public void ShouldReturnCorrectStemmingResultsForPass()
        {
            using (var store = NewDocumentStore())
            {
                new TestCommentsStandandAnalyzerIndex().Execute(store);
                ...
                using (var session = store.OpenSession())
                {
                    var results = session.Query<TestComment, TestCommentsStandandAnalyzerIndex>()
        ...
        public void ShouldReturnCorrectResultsForWordUsingIQuerableWithAppendingCriteria()
        {
            using (var store = NewDocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    var query = session.Query<TestComment, TestCommentsStandandAnalyzerIndex>().Search(x => x.Comment, "word")
